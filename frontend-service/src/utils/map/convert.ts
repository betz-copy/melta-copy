...
const getZoneAndHemi = (longitude: number, latitude: number): { epsgCode: string; zone: number; hemi: 'N' | 'S' } => {
    ...
    // Determine the correct EPSG code based on hemisphere:
    // Northern hemisphere: EPSG:326XX, Southern hemisphere: EPSG:327XX.
    const epsgCode = latitude >= 0 ? `EPSG:326${zoneNumber.toString().padStart(2, '0')}` : `EPSG:327${zoneNumber.toString().padStart(2, '0')}`;

    return { epsgCode, zone: zoneNumber, hemi: latitude >= 0 ? 'N' : 'S' };
};

const wgs84 = 'EPSG:4326';
const utm = (zone: UTM['zone'], hemi: UTM['hemi']) => `EPSG:${hemi === 'N' ? '326' : '327'}${zone}`;
...
export const convertWGS84ToUTM = (wgs84Location: Cartesian3 | Cartesian3[]): UTM | UTM[] => {
    ...
    return !Array.isArray(wgs84Location) ? convertToUTM(wgs84Location) : wgs84Location.map(convertToUTM);
};
...
export const locationConverterToString = (
    ...
    console.log({ location, unitToConvertFrom, unitToConvertTo });
