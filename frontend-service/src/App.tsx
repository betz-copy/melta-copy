...
const App: React.FC = () => {
	...
	const { isError: isErrorBackendConfig } = useQuery<BackendConfigState>(
		"getBackendConfig",
		getBackendConfigRequest,
		{
			onError: () => {
				...
				const mapLayers = await Promise.all(
					layers?.map(async (layer) => {
						...
						const endpoint = await new WmtsEndpoint(
							`${capabilitiesUrl}?url=${layerProvider.url}`,
						).isReady();
	...
	useEffect(() => {
		...
		const handleWorkspace = async () => {
			...
			if (
				...
				currentUser.permissions[workspaceStore._id]
			)
				setUser({
					...currentUser,
	...
	useEffect(() => {
		const initUser = async () => {
			...
			try {
				...
				const adminWorkspaceIds = workspaceIds.filter(
					(workspaceId) => userFromDb.permissions[workspaceId].admin,
				);

				const isAdminRoot = (
					await Promise.all(adminWorkspaceIds.map((id) => getById(id)))
				...
				if (workspaceIds.length === 1) {
					...
					if (workspace.name !== "" && workspace.path !== "/")
						navigate(
							`${path}${location.length <= path.length ? "" : location.replace(path, "")}${normalizedSearchParams}`,
						);
				}
			} catch {
