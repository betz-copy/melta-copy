...
export const Description: React.FC<IProcessReviewerUpdateNotificationMetadataPopulated> = ({ process, addedSteps, deletedSteps, unchangedSteps }) => {
        ...
        return (
            <Grid item>
                <Typography display="inline">
                    {i18next.t(`processReviewerUpdateNotification.removedFromProcess`) + ' '}
                </Typography>
    ...
    return (
        <>
            <Grid item>
                <Typography display="inline">
                    {i18next.t(`processReviewerUpdateNotification.inProcess`) + ' '}
                </Typography>
                <Typography display="inline" fontWeight="bold">
                    {process.name + ' '}
                </Typography>
                <Typography display="inline">
                    {i18next.t(`processReviewerUpdateNotification.inTheFollowingSteps`) + ' '}
                </Typography>
            </Grid>

            {Boolean(addedSteps.length) && <Grid item>
                <Typography sx={{ textDecoration: 'underline' }}>
                    {i18next.t(`processReviewerUpdateNotification.addedToReviewers`) + ':'}
                </Typography>
                {addedStepsNames.map((stepName) => <Typography key={stepName} fontWeight="bold">{stepName}</Typography>)}
            </Grid>}
            {Boolean(deletedSteps.length) && <Grid item>
                <Typography sx={{ textDecoration: 'underline' }}>
                    {i18next.t(`processReviewerUpdateNotification.removedFromReviewers`) + ':'}
                </Typography>
                {deletedStepsNames.map((stepName) => <Typography key={stepName} fontWeight="bold">{stepName}</Typography>)}
            </Grid>}
        </>
    )
}