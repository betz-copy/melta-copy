...
export const DayNightSwitch: React.FC<DayNightSwitchProps> = ({ checked, onClick, animationScale = 30, animationDuration = 0.4 }) => {
    ...
    useEffect(() => {
        ...
        stars.map((star) => gsap.to(star, { duration: 'random(0.9, 1.5)', repeat: -1, yoyo: true, opacity: 'random(0.2, 0.5)' }));
        ...
        toNightAnimation
            .to('#night-content', { duration: animationDuration * 0.5, opacity: 1, ease: 'power2.inOut', x: 0 })
            .to(
                '#circle',
                {
            ...
            .to('.day-label', { duration: animationDuration * 2, ease: 'power2.inOut', opacity: 0.2 }, 0)
            .to('.night-label', { duration: animationDuration * 2, ease: 'power2.inOut', opacity: 1 }, 0)
            .set(
                '#circle',
                {
                    scaleX: -animationScale,
                    onUpdate: () => {
                        setDay(!checkedRef.current);
                    },
                },
                animationDuration,
            )
                    ...
                    <svg id="toggle" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 34" width="60" height="34">
                        ...
                        <g clipPath="url(#container)">
                            ...
                            <g clipPath="url(#switch)">
                                <use id="front" href={`#${isDay ? 'night' : 'day'}`} />
                            </g>
