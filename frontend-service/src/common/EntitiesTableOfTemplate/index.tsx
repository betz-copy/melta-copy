...
const {
	...
	actionPrefix,
	actionsWidth,
	rowCountInfiniteModeWithoutExpand,
} = environment.agGrid;
const { columnWidths, columnsOrder, visibleColumns } =
	environment.agGrid.localStorage;
...
export const getDatasource = <Data extends EntityData>(
	...
	const parentTemplateId = isChildTemplate(template)
		? template.parentTemplate._id
		: template._id;
	const childTemplateIds = isChildTemplate(template)
		? (childTemplatesOfParentIds ?? [template._id])
		: [];
	...
	<Data extends EntityData>(
		{
			...
			hasInstances,
			multipleSelect,
			paginationPageSizeSelector = environment.agGrid
				.paginationPageSizeSelector as unknown as number[],
		...
		const entityTemplates =
			queryClient.getQueryData<IEntityTemplateMap>("getEntityTemplates")!;
		const childTemplates =
			queryClient.getQueryData<IChildTemplateMap>("getChildTemplates");
		const units = queryClient.getQueryData<IGetUnits>("getUnits")!;

		const [_, navigate] = useLocation();
		const darkMode = useDarkModeStore((state) => state.darkMode);
		const savedVisibleColumns = localStorage.getItem(
			`${visibleColumns}${saveStorageProps.pageType}-${template._id}`,
		);
		const defaultVisibleColumnsRef = useRef<Record<string, boolean>>(
		...
		const childTemplateId = isChildTemplate(template)
			? template._id
			: undefined;

		const currentUser = useUserStore((state) => state.user);
		const clientSideUserEntity = useClientSideUserStore(
			(state) => state.clientSideUserEntity,
		);
		...
		const [gridHeight, setGridHeight] = useState<number>(() =>
			infiniteModeWithoutExpand
				? rowHeight * rowCountInfiniteModeWithoutExpand
				: rowHeight * defaultExpandedRowCount,
		);
		const [selectedRow, setSelectedRow] = useState("");
		const [currEntity, setCurrEntity] = useState<IEntity>();
		const [currEditingCell, setCurrEditingCell] = useState<
			CellClickedEvent<Data> | undefined
		>();
		...
		const filteredColumns = Object.keys(
			pickBy(
				...
				(property) => property.comment === undefined,
			),
		);

		const savedColumnsOrder = localStorage.getItem(
			`${columnsOrder}${saveStorageProps.pageType}-${template._id}`,
		);
		const [defaultColumnsOrder, setDefaultColumnsOrder] = useState(
			savedColumnsOrder ? JSON.parse(savedColumnsOrder) : {},
		);
		...
		const { isLoading: isDeleteLoading, mutateAsync: deleteMutation } =
			useMutation(
				(id: string) =>
					deleteEntityRequest({
						selectAll: false,
						templateId: isChildTemplate(template)
							? template.parentTemplate._id
							: template?._id,
						idsToInclude: [id],
					...
					} as IDeleteEntityBody<false>),
				{
					onError: (error: AxiosError) => {
						toast.error(
							<ErrorToast
								axiosError={error}
								defaultErrorMessage={i18next.t("wizard.entity.failedToDelete")}
							/>,
						);
					},
					onSuccess: () => {
		...
		const { mutateAsync: updateEntityStatus } = useMutation(
			({
				...
				updateEntityStatusRequest(
					currentEntity.properties._id,
					disabled,
					JSON.stringify(ignoredRules),
					childTemplateId,
				),
			{
				onSuccess: (data) => {
					toast.success(
						i18next.t(
							`entityPage.${data.properties.disabled ? "disabled" : "activated"}Successfully`,
						),
					);
				...
				onError: (_err: AxiosError, { disabled }) => {
					toast.error(
						i18next.t(
							`entityPage.failedTo${disabled ? "Disable" : "Activate"}`,
						),
					);
				},
			},
		...
		const columnDefProps: IGetColumnDefsOptions<Data> = {
			...
			onNavigateToRow: showNavigateToRowButton
				? (data) => navigate(`/entity/${getEntityPropertiesData(data)._id}`)
				: undefined,
			deleteRowButtonProps,
			menuRowButtonProps,
			addRelationshipReferenceButtonProps,
			...
			disableEditCell:
				!editable ||
				editRowButtonProps?.disabledButton ||
		...
		const columnDefs = useDeepCompareMemo(
			() => getColumnDefs(columnDefProps),
			[columnDefProps],
		);
		const childTemplatesOfParentIds = childTemplatesOfParent?.map(
			({ _id }) => _id,
		);
		...
		const updateVisibleColumns = (
			params: ColumnVisibleEvent<Data> | GridReadyEvent<Data>,
		) => {
			const columnState = params.api.getColumnState();

			const updatedVisibleColumns = columnState.reduce<Record<string, boolean>>(
				(acc, col) => {
		...
		const handleColumnVisible = (params: ColumnVisibleEvent<Data>) => {
			...
			if (
				...
				params.column.getColId() === "disabled"
			) {
				const { disabled: _disabled, ...rest } = params.api.getFilterModel();
				const filterModel = params.column.isVisible()
					? rest
					: { ...rest, ...defaultFilterModel };
		...
		const handleColumnsOrder = (
			params:
				| ColumnMovedEvent<Data>
				| GridReadyEvent<Data>
				| FirstDataRenderedEvent<Data>
				| RowDataUpdatedEvent<Data>,
		) => {
			if (!saveStorageProps.shouldSaveColumnOrder) return;
			const columnState = params.api.getColumnState();
			const newColumnsOrder = columnState.reduce<
		...
		const handleColumnResized = (params: ColumnResizedEvent<Data>) => {
			if (
				params.finished &&
				params.column &&
				["autosizeColumns", "uiColumnDragged", "uiColumnResized"].includes(
					params.source,
				)
			) {
				const currColumnWidths = localStorage.getItem(
					`${columnWidths}${saveStorageProps.pageType}-${template._id}`,
				);
				const currColumnWidthsParsed = currColumnWidths
					? JSON.parse(currColumnWidths)
					: {};
				localStorage.setItem(
					`${columnWidths}${saveStorageProps.pageType}-${template._id}`,
					JSON.stringify({
		...
		const handleSortChanged = () => {
			...
			localStorage.setItem(
				`sortModel-${saveStorageProps.pageType}-${template._id}`,
		...
		const handlePaginationChanged = (params: PaginationChangedEvent<Data>) => {
			...
			if (params.api && params.newPage) {
				...
				sessionStorage.setItem(
					`currentPage-${saveStorageProps.pageType}-${template._id}`,
		...
		const calculateRemainingWidth = (
			...
			hasActions: boolean,
			isRemovedFields: boolean,
		): number => {
			const usedWidth: number = isRemovedFields
				? 0
				: Object.values(defaultColumnWidths).reduce(
						(sum, width) => sum + width,
						0,
					);
			const totalGridWidth: number = tableRef.current?.offsetWidth ?? 0;
			const widthConsumed: number = columnStates.reduce(
		...
		const autoSizeAll = (
			params: GridReadyEvent<Data> | RowDataUpdatedEvent<Data>,
			visibleKeys: string[],
		) => {
			const { api } = params;

			const hasActions = visibleKeys.some((key) =>
				key.startsWith(actionPrefix),
			);

			const uniqKeys = ["disabled", "createdAt", "updatedAt"];
			const defaultKeys = Object.keys(defaultColumnWidths).filter(
				(key) => !key.includes(actionPrefix) && !uniqKeys.includes(key),
			);
			const isRemovedFields = defaultKeys.some(
				(key) => !filteredColumns.includes(key),
			);

			if (
				filteredColumns.length === defaultKeys.length &&
				filteredColumns.every((key, index) => key === defaultKeys[index])
			)
				return;
			...
			const shouldIncludeKey = (key: string) =>
				key !== `${actionPrefix}${template._id}` &&
				(isRemovedFields || !defaultColumnWidths[key]);
			const columnsKeys = visibleKeys.filter(shouldIncludeKey);
			if (!columnsKeys.length) return;

			api.refreshHeader();
			api.sizeColumnsToFit();
			Object.keys(defaultColumnWidths).length
				? api.autoSizeColumns(columnsKeys)
				: api.autoSizeColumns(filteredColumns);

			const columnStates = api
				.getColumnState()
				.filter((col) => columnsKeys.includes(col.colId));

			const remainingWidth = calculateRemainingWidth(
				columnStates,
				hasActions,
				isRemovedFields,
			);
			...
			const columnsWidth: Record<string, number> = columnStates.reduce(
				(acc, col, index) => {
					const newWidth =
						index === columnStates.length - 1
							? col.width! + Math.max(remainingWidth, 0)
							: col.width!;
			...
			if (Object.keys(columnsWidth).length) {
				const updatedWidths = isRemovedFields
					? columnsWidth
		...
		const handleBodyScroll = debounce((params: BodyScrollEvent<Data>) => {
			...
			if (
				params.api.getVerticalPixelRange().top >= 0 &&
				rowModelType === "infinite"
			) {
				sessionStorage.setItem(
					`scrollPosition-${template._id}`,
					JSON.stringify(params.api.getVerticalPixelRange().top),
				);
			}
		}, 500);

		const { isLoading: isUpdateLoading, mutateAsync: updateMutation } =
			useMutation(
				({
					newEntityData,
					ignoredRules,
					...
					),
				{
					...
					onError: (err: AxiosError, { newEntityData: newEntityDate }) => {
						const errorMetadata = (err.response?.data as IErrorResponse)
							?.metadata;
						...
						switch (errorMetadata?.errorCode) {
							case errorCodes.failedConstraintsValidation:
								switch (errorMetadata.constraint.type) {
									case "REQUIRED":
										...
										break;
									case "UNIQUE": {
										const { properties } = errorMetadata.constraint as Omit<
											IUniqueConstraint,
											"constraintName"
										>;
										const constraintPropsDisplayNames = properties.map(
											(prop) =>
												`${prop}-${template.properties.properties[prop].title}`,
										);
							...
							case errorCodes.ruleBlock: {
								const { brokenRules, rawBrokenRules, actions, rawActions } =
									errorMetadata;
		...
		const statusPanels = useMemo(() => {
			const panels: StatusPanelDef[] = [
				{ statusPanel: RowCountGridStatusBar, align: "right" },
			];
				...
				>
					<AgGridReact<Data>
						...
						containerStyle={{
							width: "100%",
							height:
								rowModelType === "infinite" || !usePagination
									? `${gridHeight}px`
									: undefined,
							fontFamily: "Rubik",
							fontSize,
						...
						onBodyScroll={
							rowModelType === "infinite" ? handleBodyScroll : undefined
						}
						...
						onRowSelected={
							onRowSelected
								? ({ data, node, event }) => {
										...
										const colId = (event?.target as HTMLElement)
											?.closest("[col-id]")
											?.getAttribute("col-id");
						...
						onGridReady={(params) => {
							if (saveStorageProps.pageType === TablePageType.map) {
								...
								const visibleKeys = Object.keys(updatedVisibleColumns).filter(
									(key) => updatedVisibleColumns[key] === true,
								);
								autoSizeAll(params, visibleKeys);
							}

							const savedSortModel = localStorage.getItem(
								`sortModel-${saveStorageProps.pageType}-${template._id}`,
							);
							if (savedSortModel) {
								const sortModel: IServerSideGetRowsRequest["sortModel"] =
							...
							const savedFilterModel = LocalStorage.get(
								`tableFilter-${saveStorageProps.pageType}-${template._id}`,
							);
							if (savedFilterModel)
						...
						onFirstDataRendered={(params) => {
							...
							if (rowModelType === "infinite") {
								const savedRowIndex = sessionStorage.getItem(
									`scrollPosition-${template._id}`,
								);
						...
						statusBar={
							rowModelType === "infinite" && !infiniteModeWithoutExpand
								? { statusPanels }
								: undefined
						}
						...
						onCellEditingStopped={(params: CellEditingStoppedEvent) => {
							...
							const isEmpty =
								...
								isEmptyArray;
							const isRequired = template.properties.required.includes(
								params.colDef.field!,
							);
							const updatedProperties = {
								...params.data?.properties,
								[params.column.getColId()]: isEmpty
									? isRequired || isEmptyArray
										? undefined
										: ""
									: params.newValue,
							};
							...
							gridRef.current?.api.forEachNode((rowNode) => {
								if (
									rowNode.data &&
									getRowId(properties as Data) === getRowId(rowNode.data)
								)
									rowNode.updateData(properties as Data);
							});
						...
						onCellClicked={(params) => {
							const isHidden = template.properties.hide.includes(
								params.colDef.field!,
							);
							if (isHidden || !params.colDef.cellEditor) return;
							setCurrEditingCell(params);
							if (currEditingCell && currEditingCell.value !== params.value)
