...
const RelationshipReferenceView: React.FC<RelationshipReferenceViewProps> = ({
  ...
  const allowedChildTemplates =
    queryClient.getQueryData<IChildTemplateMap>("getChildTemplates")!;
  const childTemplatesOfRelatedTemplate =
    Array.from(allowedChildTemplates.values()).filter(
      (child) => child.parentTemplate._id === relatedTemplateId
    ) ?? [];

  const template = entityTemplates.get(relatedTemplateId!);
  const relatedEntityTemplate =
    template ?? childTemplatesOfRelatedTemplate[0]?.parentTemplate;
  const entityTemplateColor = relatedEntityTemplate
    ? getEntityTemplateColor(relatedEntityTemplate)
    : undefined;
  ...
  if (!template && !adjustedChildTemplate)
    return (
      ...
      >
        ...
        >
          <VisibilityOff
            sx={{ height, width, color: theme.palette.action.disabled }}
          />
        </MeltaTooltip>
      </Grid>
    );
  ...
  const relationshipObjectToField = (): string => {
    if (
      relatedEntityTemplate.properties.properties[relatedTemplateField]
        .format === "location"
    ) {
      return entity.properties[`${relatedTemplateField}_coordinateSystem`] ===
    ...
    if (
      relatedEntityTemplate.properties.properties[relatedTemplateField].type ===
        "array" &&
      relatedEntityTemplate.properties.properties[relatedTemplateField]?.items
        ?.format === "user"
    ) {
      ...
      if (Array.isArray(usersProperty)) {
        return entity.properties[relatedTemplateField]
          .map((user) => JSON.parse(user).fullName)
          .join(", ");
      }
