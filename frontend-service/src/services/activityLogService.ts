...
const getActivityLogRequest = async (entityId: string, limit: number, skip: number) => {
    const { data } = await axios.get<IActivityLog[]>(`${activityLog}/${entityId}`, { params: { limit, skip } });
    return data;
};
