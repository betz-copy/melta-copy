...
export const MeltaPolygon = ({
    ...
    return (
        <>
            <Entity name={name}>
                <PolylineGraphics positions={[...polygon, polygon[0]]} material={Color.WHITE} width={2} />
                <PolygonGraphics hierarchy={polygon} material={Color.fromAlpha(Color.fromCssColorString(color), 0.3)} />
                {polygon.map((position, index) => (
                    <Entity key={`${position.x}, ${position.y} - ${index}`} position={position}>
                        <PointGraphics color={Color.WHITE} outlineColor={Color.WHITE} pixelSize={2} outlineWidth={2} />
                    </Entity>
                ))}
            </Entity>

            <Entity name={name} description={locationToWGS84String(polygon)} position={centroid} onClick={onClick}>
                <PointGraphics color={Color.fromCssColorString(color)} outlineColor={Color.WHITE} pixelSize={12} outlineWidth={2} />
            </Entity>
        </>
    );
};

const getColoredLocationIcon = (color: string = COORDINATE_DEFAULT_COLOR) => {
    const svg = `
