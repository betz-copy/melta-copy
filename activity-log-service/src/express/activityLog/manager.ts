...
export class ActivityLogManager {
    static async getActivity(entityId: string, limit: number, skip: number, action?: string) {            
        if(action)
           return ActivityLogModel.find({ entityId: entityId, action: new RegExp(action!) }).limit(limit).skip(skip).exec()   
