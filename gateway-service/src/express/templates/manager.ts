...
export class TemplatesManager {
    ...
    static async getAllAllowedTemplates(userId: string, permissionsOfUserId: Omit<IPermissionsOfUser, 'user'>) {
        const filename1 = 'test.ts';
        const code = `const test: number = 1 + 2 ; console.log(y) ;`;

        const sourceFile = ts.createSourceFile(filename1, code, ts.ScriptTarget.Latest);
        ...
        // with creating file and know words as console log etc...

        // const code = 'let x = 5 ; x += 2;  console.log(u); y() ; ss+=5; ';
        // const sourceFileName = 'example.ts';
