...
class FlowCubeController extends DefaultController<FlowCubeManager> {
    ...
    async searchFlowCube(req: Request, res: Response) {
        res.json(await this.manager.searchFlowCube(req.params.workspaceId, req.params.templateId, req.body));
    }
    ...
    async searchEntityTemplate(req: Request, res: Response) {
        res.json(await this.manager.searchEntityTemplate(req.body, req.user?.id));
    }
