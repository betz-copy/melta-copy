...
export class Authorizer extends DefaultController {
    ...
    async getWorkspacePermissions(userId: string) {
        ...
        const userPermissions = await UserService.getUserPermissions(userId, RelatedPermission.User, workspaceHierarchyIds);
    ...
    async userIsRootAdmin(req: Request) {
        ...
        const userPermissions = await UserService.getUserPermissions(req.user!.id, RelatedPermission.User, [rootWorkspace._id]);
