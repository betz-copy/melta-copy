import { IGantt, IMongoGantt, ISearchGanttsBody } from '@microservices/shared-interfaces';
...
export const createGantt = async (gantt: IGantt) => {
    const { data } = await axios.post<IMongoGantt>(gantts, gantt);
    return data;
};

export const deleteGantt = async (ganttId: string) => {
    const { data } = await axios.delete<IMongoGantt>(`${gantts}/${ganttId}`);
    return data;
};
